...
class KDTree
{
  ...
  private:
     ...
     */
     Point<Dim> findNearestNeighbor(KDTreeNode* node, const Point<Dim>& query, int depth) const;
     bool closest(const Point<Dim>& target, const Point<Dim>& p1, const Point<Dim>& p2) const;
     double squaredD(const Point<Dim>& p1, const Point<Dim>& p2) const;
     KDTreeNode* treeHelper(int left, int right, int depth);
     KDTreeNode* copier(KDTreeNode* node);
