  ...
  */
  ...
  public:
    ...
    */
    ...
    public:
      Iterator();
      Iterator(PNG png, const Point & start, double tolerance, TraversalFunctions fns);
      Iterator & operator++();
      Point operator*();
      ...
      //unsigned long encodePoint(const Point& p) const;
    private:
      /** @todo [Part 1] */
      ...
      std::unordered_set<unsigned long> visited_;

      unsigned long encodePoint(const Point& p) const;

      [[maybe_unused]]  double tolerance_;
      [[maybe_unused]] TraversalFunctions fns_;
